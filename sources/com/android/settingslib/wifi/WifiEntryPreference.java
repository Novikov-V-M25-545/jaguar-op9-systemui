...
public class WifiEntryPreference extends Preference implements WifiEntry.WifiEntryCallback, View.OnClickListener {
    ...
    public void onBindViewHolder(PreferenceViewHolder preferenceViewHolder) {
        ...
        if (this.mWifiEntry.getHelpUriString() != null && this.mWifiEntry.getConnectedState() == 0) {
            ...
            drawable.setTintList(Utils.getColorAttr(getContext(), R.attr.colorControlNormal));
    ...
    public void refresh() {
        ...
        boolean zShouldShowXLevelIcon = this.mWifiEntry.shouldShowXLevelIcon();
        if (level != this.mLevel || zShouldShowXLevelIcon != this.mShowX) {
            this.mLevel = level;
            this.mShowX = zShouldShowXLevelIcon;
            updateIcon(zShouldShowXLevelIcon, level);
            notifyChanged();
        }
    ...
    private StateListDrawable getFrictionStateListDrawable() {
        ...
        try {
            typedArrayObtainStyledAttributes = getContext().getTheme().obtainStyledAttributes(FRICTION_ATTRS);
