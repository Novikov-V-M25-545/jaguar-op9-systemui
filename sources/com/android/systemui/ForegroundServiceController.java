...
public class ForegroundServiceController {
    ...
    public /* synthetic */ void lambda$new$1(final int i, final int i2, final String str, final boolean z) {
        this.mMainHandler.post(new Runnable() { // from class: com.android.systemui.ForegroundServiceController$$ExternalSyntheticLambda1
            ...
            public final void run() {
                this.f$0.lambda$new$0(i, i2, str, z);
            }
        });
    }
            ...
            while (it.hasNext()) {
                ...
                if (pendingOrActiveNotif != null && i2 == pendingOrActiveNotif.getSbn().getUid() && str.equals(pendingOrActiveNotif.getSbn().getPackageName())) {
                    synchronized (pendingOrActiveNotif.mActiveAppOps) {
                        if (z) {
                            zAdd = pendingOrActiveNotif.mActiveAppOps.add(Integer.valueOf(i));
                        } else {
                            zAdd = pendingOrActiveNotif.mActiveAppOps.remove(Integer.valueOf(i));
                        }
                        z2 |= zAdd;
                    }
                }
    ...
    boolean updateUserState(int i, UserStateUpdateCallback userStateUpdateCallback, boolean z) {
        synchronized (this.mMutex) {
            ForegroundServicesUserState foregroundServicesUserState = this.mUserServices.get(i);
            if (foregroundServicesUserState == null) {
                if (!z) {
