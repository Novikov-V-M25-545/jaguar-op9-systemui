...
public class NotificationMenuRow implements NotificationMenuRowPlugin, View.OnClickListener, ExpandableNotificationRow.LayoutListener {
    ...
    public void onNotificationUpdated(StatusBarNotification statusBarNotification) {
        ...
        createMenuViews(!isMenuVisible(), (statusBarNotification.getNotification().flags & 64) != 0);
    }
    ...
    public void onParentTranslationUpdate(float f) {
        ...
        float width2 = 0.0f;
        if (fAbs != 0.0f) {
            width2 = fAbs <= width ? 1.0f : 1.0f - ((fAbs - width) / (this.mParent.getWidth() - width));
        }
        setMenuAlpha(width2);
    }
    ...
    public void fadeInMenu(final float f) {
        ...
        valueAnimatorOfFloat.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() { // from class: com.android.systemui.statusbar.notification.row.NotificationMenuRow.1
            ...
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                ...
                boolean z2 = z;
                if (!((z2 && f2 <= f) || (!z2 && fAbs <= f)) || NotificationMenuRow.this.mMenuFadedIn) {
                    return;
                }
