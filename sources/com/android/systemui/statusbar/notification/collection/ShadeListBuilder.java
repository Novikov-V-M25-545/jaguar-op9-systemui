...
public class ShadeListBuilder implements Dumpable {
    private static final Comparator<NotificationEntry> sChildComparator = new Comparator() { // from class: com.android.systemui.statusbar.notification.collection.ShadeListBuilder$$ExternalSyntheticLambda5
    ...
    private final Comparator<ListEntry> mTopLevelComparator = new Comparator() { // from class: com.android.systemui.statusbar.notification.collection.ShadeListBuilder$$ExternalSyntheticLambda4
        @Override // java.util.Comparator
        public final int compare(Object obj, Object obj2) {
    ...
    void setSections(List<NotifSection> list) {
        Assert.isMainThread();
        this.mPipelineState.requireState(0);
        this.mNotifSections.clear();
        for (NotifSection notifSection : list) {
            this.mNotifSections.add(notifSection);
            notifSection.setInvalidationListener(new Pluggable.PluggableListener() { // from class: com.android.systemui.statusbar.notification.collection.ShadeListBuilder$$ExternalSyntheticLambda3
                @Override // com.android.systemui.statusbar.notification.collection.listbuilder.pluggable.Pluggable.PluggableListener
                public final void onPluggableInvalidated(Object obj) {
                    this.f$0.onNotifSectionInvalidated((NotifSection) obj);
                }
            });
        }
    }
    ...
    private void promoteNotifs(final List<ListEntry> list) {
        for (int i = 0; i < list.size(); i++) {
            ...
            if (listEntry instanceof GroupEntry) {
                ((GroupEntry) listEntry).getRawChildren().removeIf(new Predicate() { // from class: com.android.systemui.statusbar.notification.collection.ShadeListBuilder$$ExternalSyntheticLambda6
    ...
    private void sortList() {
        for (ListEntry listEntry : this.mNotifList) {
            Pair<NotifSection, Integer> pairApplySections = applySections(listEntry);
            if (listEntry instanceof GroupEntry) {
                GroupEntry groupEntry = (GroupEntry) listEntry;
                for (NotificationEntry notificationEntry : groupEntry.getChildren()) {
                    notificationEntry.getAttachState().setSection((NotifSection) pairApplySections.first);
                    notificationEntry.getAttachState().setSectionIndex(((Integer) pairApplySections.second).intValue());
                }
    ...
    public /* synthetic */ int lambda$new$2(ListEntry listEntry, ListEntry listEntry2) {
        int iCompare = Integer.compare(listEntry.getSection(), listEntry2.getSection());
        if (iCompare == 0) {
            for (int i = 0; i < this.mNotifComparators.size() && (iCompare = this.mNotifComparators.get(i).compare(listEntry, listEntry2)) == 0; i++) {
            }
        }
        NotificationEntry representativeEntry = listEntry.getRepresentativeEntry();
        NotificationEntry representativeEntry2 = listEntry2.getRepresentativeEntry();
        if (iCompare == 0) {
            iCompare = representativeEntry.getRanking().getRank() - representativeEntry2.getRanking().getRank();
        }
        return iCompare == 0 ? Long.compare(representativeEntry2.getSbn().getNotification().when, representativeEntry.getSbn().getNotification().when) : iCompare;
    }
    ...
    private Pair<NotifSection, Integer> applySections(ListEntry listEntry) {
        Pair<NotifSection, Integer> pairFindSection = findSection(listEntry);
        NotifSection notifSection = (NotifSection) pairFindSection.first;
        Integer num = (Integer) pairFindSection.second;
        listEntry.getAttachState().setSection(notifSection);
        listEntry.getAttachState().setSectionIndex(num.intValue());
        return pairFindSection;
    }
