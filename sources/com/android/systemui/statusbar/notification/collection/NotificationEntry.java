...
public final class NotificationEntry extends ListEntry {
    ...
    public boolean isBubble() {
        return (this.mSbn.getNotification().flags & LineageHardwareManager.FEATURE_AUTO_CONTRAST) != 0;
    }
    ...
    public boolean setFlagBubble(boolean z) {
        boolean zIsBubble = isBubble();
        if (!z) {
            this.mSbn.getNotification().flags &= -4097;
        } else if (this.mBubbleMetadata != null && canBubble()) {
            this.mSbn.getNotification().flags |= LineageHardwareManager.FEATURE_AUTO_CONTRAST;
        }
        return zIsBubble != isBubble();
    }
    ...
    public boolean secureContent() {
        return (!this.mIsAppLocked || this.mAppOpen || this.mIsAlarmOrCall) ? false : true;
    }

    private void updateIsAlarmOrCall() {
        PendingIntent pendingIntent = this.mSbn.getNotification().contentIntent;
        ...
        if (component != null) {
            ...
            this.mIsAlarmOrCall = lowerCase.contains("callactivity") || lowerCase.contains("callingactivity") || lowerCase.contains("voipactivity") || lowerCase.contains("alarmactivity");
        } else if (this.mSbn.getNotification().fullScreenIntent != null) {
    ...
    public boolean shouldSuppressNotificationList() {
        return shouldSuppressVisualEffect(LineageHardwareManager.FEATURE_SUNLIGHT_ENHANCEMENT);
    }
