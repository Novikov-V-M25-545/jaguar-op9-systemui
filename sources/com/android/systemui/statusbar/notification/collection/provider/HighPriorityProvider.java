...
public class HighPriorityProvider {
    ...
    private boolean hasHighPriorityChild(ListEntry listEntry) {
        ...
        if (listEntry instanceof GroupEntry) {
            children = ((GroupEntry) listEntry).getChildren();
        } else {
            children = (listEntry.getRepresentativeEntry() == null || !this.mGroupManager.isGroupSummary(listEntry.getRepresentativeEntry().getSbn())) ? null : this.mGroupManager.getChildren(listEntry.getRepresentativeEntry().getSbn());
        }
        if (children == null) {
            return false;
        }
        Iterator<NotificationEntry> it = children.iterator();
        while (it.hasNext()) {
            if (isHighPriority(it.next())) {
                return true;
            }
        }
        return false;
