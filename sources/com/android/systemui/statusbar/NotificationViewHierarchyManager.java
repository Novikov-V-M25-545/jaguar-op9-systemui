...
public class NotificationViewHierarchyManager implements DynamicPrivacyController.Listener {
    ...
    public void updateNotificationViews() {
        ...
        while (true) {
            ...
            if (!notificationEntry.isRowDismissed() && !notificationEntry.isRowRemoved() && !this.mBubbleController.isBubbleNotificationSuppressedFromShade(notificationEntry) && !this.mFgsSectionController.hasEntry(notificationEntry)) {
                ...
                boolean zIsLockscreenPublicMode = this.mLockscreenUserManager.isLockscreenPublicMode(currentUserId);
                boolean z2 = zIsLockscreenPublicMode || this.mLockscreenUserManager.isLockscreenPublicMode(userId);
                ...
                boolean zSecureContent = notificationEntry.secureContent();
                boolean z3 = zSecureContent || this.mLockscreenUserManager.needsRedaction(notificationEntry);
                notificationEntry.setSensitive(z2 && z3, zIsLockscreenPublicMode && (zSecureContent || !this.mLockscreenUserManager.userAllowsPrivateNotificationsInPublic(currentUserId)));
                ...
                boolean zIsChildInGroupWithSummary = this.mGroupManager.isChildInGroupWithSummary(notificationEntry.getSbn());
                ...
                if (!z) {
                    boolean zIsChildInGroup = notificationEntry.isChildInGroup();
                    if (zIsChildInGroupWithSummary && !zIsChildInGroup) {
                        this.mVisualStabilityManager.addGroupChangesAllowedCallback(this.mEntryManager, false);
                    } else if (!zIsChildInGroupWithSummary && zIsChildInGroup && this.mGroupManager.isLogicalGroupExpanded(notificationEntry.getSbn())) {
                        groupSummary = notificationEntry.getRow().getNotificationParent().getEntry();
                        this.mVisualStabilityManager.addGroupChangesAllowedCallback(this.mEntryManager, false);
                    }
                    zIsChildInGroupWithSummary = zIsChildInGroup;
                }
                if (zIsChildInGroupWithSummary) {
                    List<NotificationEntry> arrayList2 = this.mTmpChildOrderMap.get(groupSummary);
                    if (arrayList2 == null) {
        ...
        for (int i = 0; i < this.mListContainer.getContainerChildCount(); i++) {
            ...
            if (containerChildAt instanceof ExpandableNotificationRow) {
                ...
                if (list != null) {
                    ...
                    zApplyChildOrder |= expandableNotificationRow.applyChildOrder(arrayList, this.mVisualStabilityManager, this.mEntryManager);
                    arrayList.clear();
                }
            }
        }
    ...
    private void updateRowStatesInternal() {
        ...
        while (!stack.isEmpty()) {
            ...
            boolean zIsChildInGroupWithSummary = this.mGroupManager.isChildInGroupWithSummary(entry.getSbn());
            ...
            boolean zShouldShowOnKeyguard = this.mLockscreenUserManager.shouldShowOnKeyguard(entry);
            if (!zShouldShowOnKeyguard && this.mGroupManager.isChildInGroupWithSummary(entry.getSbn()) && (logicalGroupSummary = this.mGroupManager.getLogicalGroupSummary(entry.getSbn())) != null && this.mLockscreenUserManager.shouldShowOnKeyguard(logicalGroupSummary)) {
                zShouldShowOnKeyguard = true;
            }
            if (z2 || this.mLockscreenUserManager.shouldHideNotifications(userId) || (z && !zShouldShowOnKeyguard)) {
                ...
                if (!zIsChildInGroupWithSummary && !entry.getRow().isRemoved()) {
                    if (z3) {
                        this.mListContainer.generateAddAnimation(entry.getRow(), !zShouldShowOnKeyguard);
                    }
                    i2++;
                }
            }
