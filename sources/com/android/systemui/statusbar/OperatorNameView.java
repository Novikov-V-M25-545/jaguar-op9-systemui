...
public class OperatorNameView extends TextView implements DemoMode, DarkIconDispatcher.DarkReceiver, NetworkController.SignalCallback, TunerService.Tunable {
    ...
    private void update() {
        setVisibility(((TunerService) Dependency.get(TunerService.class)).getValue("show_operator_name", 1) != 0 ? 0 : 8);
        boolean zIsNetworkSupported = ConnectivityManager.from(((TextView) this).mContext).isNetworkSupported(0);
        boolean zIsAirplaneModeOn = WirelessUtils.isAirplaneModeOn(((TextView) this).mContext);
    ...
    public void updateText() {
        ...
        while (true) {
            ...
            carrierName = filteredSubscriptionInfo.get(i).getCarrierName();
            if (!TextUtils.isEmpty(carrierName) && simState == 5 && (serviceState = this.mKeyguardUpdateMonitor.getServiceState(subscriptionId)) != null && serviceState.getState() == 0) {
                break;
            } else {
                i++;
            }
