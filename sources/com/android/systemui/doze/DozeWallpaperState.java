...
public class DozeWallpaperState implements DozeMachine.Part {
    ...
    public void transitionTo(DozeMachine.State state, DozeMachine.State state2) {
        ...
        if (z) {
            ...
            zShouldControlScreenOff = ((this.mDozeParameters.getDisplayNeedsBlanking() ^ true) && !this.mBiometricUnlockController.unlockedByWakeAndUnlock()) || (state == DozeMachine.State.DOZE_PULSING && state2 == DozeMachine.State.FINISH);
        }
        if (z != this.mIsAmbientMode) {
            this.mIsAmbientMode = z;
            if (this.mWallpaperManagerService != null) {
                long j = zShouldControlScreenOff ? 500L : 0L;
                try {
                    if (DEBUG) {
