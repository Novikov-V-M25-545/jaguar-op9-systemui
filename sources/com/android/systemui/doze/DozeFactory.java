...
public class DozeFactory {
    ...
    DozeMachine assembleMachine(DozeService dozeService) {
        ...
        DelayedWakeLock delayedWakeLockBuild = this.mDelayedWakeLockBuilder.setHandler(this.mHandler).setTag("Doze").build();
        DozeMachine.Service serviceWrapIfNeeded = DozeSuspendScreenStatePreventingAdapter.wrapIfNeeded(DozeScreenStatePreventingAdapter.wrapIfNeeded(new DozeBrightnessHostForwarder(dozeService, this.mDozeHost), this.mDozeParameters), this.mDozeParameters);
        DozeMachine dozeMachine = new DozeMachine(serviceWrapIfNeeded, ambientDisplayConfiguration, delayedWakeLockBuild, this.mWakefulnessLifecycle, this.mBatteryController, this.mDozeLog, this.mDockManager, this.mDozeHost);
        dozeMachine.setParts(new DozeMachine.Part[]{new DozePauser(this.mHandler, dozeMachine, this.mAlarmManager, this.mDozeParameters.getPolicy()), new DozeFalsingManagerAdapter(this.mFalsingManager), createDozeTriggers(dozeService, this.mAsyncSensorManager, this.mDozeHost, this.mAlarmManager, ambientDisplayConfiguration, this.mDozeParameters, delayedWakeLockBuild, dozeMachine, this.mDockManager, this.mDozeLog, this.mProximityCheck), createDozeUi(dozeService, this.mDozeHost, delayedWakeLockBuild, dozeMachine, this.mHandler, this.mAlarmManager, this.mDozeParameters, this.mDozeLog), new DozeScreenState(serviceWrapIfNeeded, this.mHandler, this.mDozeHost, this.mDozeParameters, delayedWakeLockBuild), createDozeScreenBrightness(dozeService, serviceWrapIfNeeded, this.mAsyncSensorManager, this.mDozeHost, this.mDozeParameters, this.mHandler), new DozeWallpaperState(this.mWallpaperManager, this.mBiometricUnlockController, this.mDozeParameters), new DozeDockHandler(ambientDisplayConfiguration, dozeMachine, this.mDockManager), new DozeAuthRemover(dozeService)});
        return dozeMachine;
    }
