...
public class SplitDisplayLayout {
    ...
    DividerSnapAlgorithm getMinimizedSnapAlgorithm(boolean z) {
    ...
    void resizeSplits(int i, WindowContainerTransaction windowContainerTransaction) {
        resizeSplits(i);
    ...
    void updateAdjustedBounds(int i, int i2, int i3) {
    ...
    private void adjustForIME(DisplayLayout displayLayout, int i, int i2, int i3, int i4, int i5, Rect rect, Rect rect2) {
        ...
        float f = (i - i2) / (i3 - i2);
        int i6 = (int) ((i5 * f) + (i4 * (1.0f - f)));
        int i7 = rect3.top;
        int iMax = Math.max(0, displayLayout.height() - (i + Math.max(0, (i2 - i3) - (this.mPrimary.bottom - (i7 + ((int) ((r1 - i7) * 0.3f)))))));
    ...
    static int getSmallestWidthDpForBounds(Context context, DisplayLayout displayLayout, Rect rect) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            DividerSnapAlgorithm dividerSnapAlgorithmInitSnapAlgorithmForRotation = initSnapAlgorithmForRotation(context, displayLayout2, dividerSize);
            rect2.set(rect);
            ...
            DockedDividerUtils.calculateBoundsForPosition(dividerSnapAlgorithmInitSnapAlgorithmForRotation.calculateNonDismissingSnapTarget(DockedDividerUtils.calculatePositionForBounds(rect2, primarySplitSide, dividerSize)).position, primarySplitSide, rect2, displayLayout2.width(), displayLayout2.height(), dividerSize);
            Rect rect5 = new Rect(rect3);
            rect5.inset(displayLayout2.stableInsets());
            rect2.intersect(rect5);
            iMin = Math.min(rect2.width(), iMin);
        }
        return (int) (iMin / displayLayout.density());
    }
