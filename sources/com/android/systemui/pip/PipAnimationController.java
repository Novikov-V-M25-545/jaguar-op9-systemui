...
public class PipAnimationController {
    ...
    private ThreadLocal<AnimationHandler> mSfAnimationHandlerThreadLocal = ThreadLocal.withInitial(new Supplier() { // from class: com.android.systemui.pip.PipAnimationController$$ExternalSyntheticLambda0
    ...
    public static class PipAnimationCallback {
        ...
        public void onPipAnimationEnd(SurfaceControl.Transaction transaction, PipTransitionAnimator pipTransitionAnimator) {
            throw null;
        }
    ...
    PipAnimationController(PipSurfaceTransactionHelper pipSurfaceTransactionHelper) {
        this.mSurfaceTransactionHelper = pipSurfaceTransactionHelper;
    }

    PipTransitionAnimator getAnimator(SurfaceControl surfaceControl, Rect rect, float f, float f2) {
        PipTransitionAnimator pipTransitionAnimator = this.mCurrentAnimator;
        if (pipTransitionAnimator == null) {
            this.mCurrentAnimator = setupPipTransitionAnimator(PipTransitionAnimator.ofAlpha(surfaceControl, rect, f, f2));
    ...
    PipTransitionAnimator getAnimator(SurfaceControl surfaceControl, Rect rect, Rect rect2, Rect rect3) {
        PipTransitionAnimator pipTransitionAnimator = this.mCurrentAnimator;
        if (pipTransitionAnimator == null) {
            this.mCurrentAnimator = setupPipTransitionAnimator(PipTransitionAnimator.ofBounds(surfaceControl, rect, rect2, rect3));
        } else if (pipTransitionAnimator.getAnimationType() == 1 && this.mCurrentAnimator.isRunning()) {
    ...
    public static abstract class PipTransitionAnimator<T> extends ValueAnimator implements ValueAnimator.AnimatorUpdateListener, Animator.AnimatorListener {
        ...
        public void onAnimationEnd(Animator animator) {
            ...
            SurfaceControl.Transaction transactionNewSurfaceControlTransaction = newSurfaceControlTransaction();
            onEndTransaction(this.mLeash, transactionNewSurfaceControlTransaction);
            PipAnimationCallback pipAnimationCallback = this.mPipAnimationCallback;
            if (pipAnimationCallback != null) {
                pipAnimationCallback.onPipAnimationEnd(transactionNewSurfaceControlTransaction, this);
            }
        }
        ...
        PipTransitionAnimator<T> setPipAnimationCallback(PipAnimationCallback pipAnimationCallback) {
        ...
        PipTransitionAnimator<T> setTransitionDirection(int i) {
            if (i != 1) {
        ...
        void setDestinationBounds(Rect rect) {
        ...
        @VisibleForTesting
        void setSurfaceControlTransactionFactory(PipSurfaceTransactionHelper.SurfaceControlTransactionFactory surfaceControlTransactionFactory) {
        ...
        static PipTransitionAnimator<Float> ofAlpha(SurfaceControl surfaceControl, Rect rect, float f, float f2) {
            return new PipTransitionAnimator<Float>(surfaceControl, 1, rect, Float.valueOf(f), Float.valueOf(f2)) { // from class: com.android.systemui.pip.PipAnimationController.PipTransitionAnimator.1
                ...
                void applySurfaceControlTransaction(SurfaceControl surfaceControl2, SurfaceControl.Transaction transaction, float f3) {
                    float fFloatValue = (((Float) getStartValue()).floatValue() * (1.0f - f3)) + (((Float) getEndValue()).floatValue() * f3);
                    setCurrentValue(Float.valueOf(fFloatValue));
                    getSurfaceTransactionHelper().alpha(transaction, surfaceControl2, fFloatValue);
                    transaction.apply();
                }
        ...
        static PipTransitionAnimator<Rect> ofBounds(SurfaceControl surfaceControl, Rect rect, Rect rect2, Rect rect3) {
            ...
            return new PipTransitionAnimator<Rect>(surfaceControl, 0, rect2, new Rect(rect), new Rect(rect2)) { // from class: com.android.systemui.pip.PipAnimationController.PipTransitionAnimator.2
                ...
                @Override // com.android.systemui.pip.PipAnimationController.PipTransitionAnimator
                void applySurfaceControlTransaction(SurfaceControl surfaceControl2, SurfaceControl.Transaction transaction, float f) {
                    Rect startValue = getStartValue();
                    Rect endValue = getEndValue();
                    Rect rectEvaluate = this.mRectEvaluator.evaluate(f, startValue, endValue);
                    setCurrentValue(rectEvaluate);
                    if (inScaleTransition()) {
                        if (PipAnimationController.isOutPipDirection(getTransitionDirection())) {
                            getSurfaceTransactionHelper().scale(transaction, surfaceControl2, endValue, rectEvaluate);
                        } else {
                            getSurfaceTransactionHelper().scale(transaction, surfaceControl2, startValue, rectEvaluate);
                        }
                    } else {
                        Rect rect7 = rect5;
                        if (rect7 != null) {
                            getSurfaceTransactionHelper().scaleAndCrop(transaction, surfaceControl2, rect4, rectEvaluate, this.mInsetsEvaluator.evaluate(f, rect6, rect7));
                        } else {
                            getSurfaceTransactionHelper().scale(transaction, surfaceControl2, startValue, rectEvaluate);
                        }
                    }
                ...
                @Override // com.android.systemui.pip.PipAnimationController.PipTransitionAnimator
                void onEndTransaction(SurfaceControl surfaceControl2, SurfaceControl.Transaction transaction) {
                    getSurfaceTransactionHelper().resetScale(transaction, surfaceControl2, getDestinationBounds()).crop(transaction, surfaceControl2, getDestinationBounds());
                }
