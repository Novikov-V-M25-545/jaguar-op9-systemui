...
class Bubble implements BubbleViewProvider {
    ...
    Bubble(NotificationEntry notificationEntry, BubbleController.NotificationSuppressionChangedListener notificationSuppressionChangedListener, final BubbleController.PendingIntentCanceledListener pendingIntentCanceledListener) {
        this.mShowBubbleUpdateDot = true;
        this.mAppUid = -1;
        Objects.requireNonNull(notificationEntry);
        this.mKey = notificationEntry.getKey();
        this.mSuppressionListener = notificationSuppressionChangedListener;
        this.mIntentCancelListener = new PendingIntent.CancelListener() { // from class: com.android.systemui.bubbles.Bubble$$ExternalSyntheticLambda0
            public final void onCancelled(PendingIntent pendingIntent) {
                this.f$0.lambda$new$0(pendingIntentCanceledListener, pendingIntent);
            }
        };
    ...
    public /* synthetic */ void lambda$new$0(BubbleController.PendingIntentCanceledListener pendingIntentCanceledListener, PendingIntent pendingIntent) {
    ...
    void inflate(BubbleViewInfoTask.Callback callback, Context context, BubbleStackView bubbleStackView, BubbleIconFactory bubbleIconFactory, boolean z) {
        if (isBubbleLoading()) {
    ...
    void setViewInfo(BubbleViewInfoTask.BubbleViewInfo bubbleViewInfo) {
        if (!isInflated()) {
    ...
    void setEntry(NotificationEntry notificationEntry) {
    ...
    void setSuppressNotification(boolean z) {
        ...
        if (showInShade() == zShowInShade || (notificationSuppressionChangedListener = this.mSuppressionListener) == null) {
            return;
        }
    ...
    void setSuppressFlyout(boolean z) {
    ...
    float getDesiredHeight(Context context) {
        if (this.mDesiredHeightResId != 0) {
            return getDimenForPackageUser(context, r0, this.mPackageName, this.mUser.getIdentifier());
        }
    ...
    private int getUid(Context context) {
        ...
        PackageManager packageManagerForUser = StatusBar.getPackageManagerForUser(context, this.mUser.getIdentifier());
    ...
    void setShouldAutoExpand(boolean z) {
        if (z) {
            enable(1);
        } else {
            disable(1);
