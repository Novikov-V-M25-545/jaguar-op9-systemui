...
public class InvocationLightsView extends View implements NavigationBarTransitions.DarkIntensityListener {
    ...
    public InvocationLightsView(Context context, AttributeSet attributeSet, int i, int i2) {
        ...
        int iConvertDpToPx = DisplayUtils.convertDpToPx(3.0f, context);
        this.mStrokeWidth = iConvertDpToPx;
        paint.setStrokeWidth(iConvertDpToPx);
        paint.setStyle(Paint.Style.STROKE);
        paint.setStrokeJoin(Paint.Join.MITER);
        paint.setAntiAlias(true);
        this.mGuide = new PerimeterPathGuide(context, createCornerPathRenderer(context), iConvertDpToPx / 2, DisplayUtils.getWidth(context), DisplayUtils.getHeight(context));
    ...
    public void onInvocationProgress(float f) {
        if (f == 0.0f) {
            ...
            float fLerp = MathUtils.lerp(0.0f, perimeterPathGuide.getRegionWidth(region) / 4.0f, f);
            float f3 = 1.0f - f;
    ...
    protected void updateDarkness(float f) {
        if (this.mUseNavBarColor) {
            ...
            Iterator<EdgeLight> it = this.mAssistInvocationLights.iterator();
            while (it.hasNext()) {
                color &= it.next().setColor(iIntValue);
            }
            if (color) {
                invalidate();
            }
