...
public abstract class CornerPathRenderer {
    ...
    private ArrayList<PointF> getApproximatePoints(Path path) {
        float[] fArrApproximate = path.approximate(0.1f);
    ...
    private ArrayList<PointF> shiftBy(ArrayList<PointF> arrayList, float f) {
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            ...
            PointF pointFNormalAt = normalAt(arrayList, i);
            arrayList2.add(new PointF(pointF.x + (pointFNormalAt.x * f), pointF.y + (pointFNormalAt.y * f)));
        }
        return arrayList2;
    }
    ...
    private PointF normalize(PointF pointF) {
        float fMagnitude = magnitude(pointF);
        if (fMagnitude == 0.0f) {
            return pointF;
        }
