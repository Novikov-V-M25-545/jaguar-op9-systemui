...
public final class AuxiliaryPersistenceWrapper {
    ...
    public final void initialize() {
        List<StructureInfo> listEmptyList;
        if (this.persistenceWrapper.getFileExists()) {
            listEmptyList = this.persistenceWrapper.readFavorites();
        } else {
            listEmptyList = CollectionsKt__CollectionsKt.emptyList();
        }
        this.favorites = listEmptyList;
    }
    ...
    public static final class DeletionJobService extends JobService {
        ...
        public static final class Companion {
            ...
            public final JobInfo getJobForContext(@NotNull Context context) {
                ...
                JobInfo jobInfoBuild = new JobInfo.Builder(getDELETE_FILE_JOB_ID$frameworks__base__packages__SystemUI__android_common__SystemUI_core() + context.getUserId(), new ComponentName(context, (Class<?>) DeletionJobService.class)).setMinimumLatency(DeletionJobService.WEEK_IN_MILLIS).setPersisted(true).build();
                Intrinsics.checkExpressionValueIsNotNull(jobInfoBuild, "JobInfo.Builder(jobId, c…                 .build()");
                return jobInfoBuild;
            }
        }
