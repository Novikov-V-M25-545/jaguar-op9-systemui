...
public class TrackedCollections {
    ...
    private static class CollectionState {
        ...
        void dump(PrintWriter printWriter) {
            ...
            printWriter.format("%s: %.2f (start-30min) / %.2f (30min-now) / %.2f (start-now) (growth rate in #/hour); %d (current size)", this.tag, Float.valueOf(ratePerHour(j, 0, j + 1800000, this.halfwayCount)), Float.valueOf(ratePerHour(this.startUptime + 1800000, this.halfwayCount, jUptimeMillis, this.lastCount)), Float.valueOf(ratePerHour(this.startUptime, 0, jUptimeMillis, this.lastCount)), Integer.valueOf(this.lastCount));
        }
    }
