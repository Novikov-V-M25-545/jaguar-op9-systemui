...
public class AssistantInvocationLightsView extends InvocationLightsView {
    ...
    public void onInvocationProgress(float f) {
        if (f <= 1.0f) {
            ...
            float fLerp = MathUtils.lerp((this.mGuide.getRegionWidth(PerimeterPathGuide.Region.BOTTOM_LEFT) * 0.6f) / 2.0f, regionWidth, 1.0f - (f - 1.0f));
            setLight(0, regionWidth - fLerp, regionWidth);
            float f2 = 2.0f * regionWidth;
            setLight(1, regionWidth, f2);
            float f3 = regionWidth * 3.0f;
            setLight(2, f2, f3);
            setLight(3, f3, fLerp + f3);
            setVisibility(0);
        }
        invalidate();
    }
