...
public class LocationBasedViewTracker implements Runnable, View.OnLayoutChangeListener {
    ...
    public void run() {
        ...
        while (it.hasNext()) {
            ...
            if (this.mFocusRect.intersect(rect) && i > (iAbs = Math.abs(this.mFocusRect.left - rect.left) + Math.abs(this.mFocusRect.right - rect.right) + Math.abs(this.mFocusRect.top - rect.top) + Math.abs(this.mFocusRect.bottom - rect.bottom))) {
                view = next;
                i = iAbs;
            }
        }
    ...
    public static void trackInputFocused(ViewGroup viewGroup) {
        ...
        if (viewFindFocus != null) {
            new LocationBasedViewTracker(viewGroup, viewFindFocus, INPUT_FOCUS);
        }
    }
