...
public class ListContent extends SliceContent {
    ...
    private void populate(Slice slice) {
        ...
        SliceItem sliceItemFindHeaderItem = findHeaderItem(slice);
        if (sliceItemFindHeaderItem != null) {
            RowContent rowContent = new RowContent(sliceItemFindHeaderItem, 0);
    ...
    public static int getListHeight(List<SliceContent> list, SliceStyle sliceStyle, SliceViewPolicy sliceViewPolicy) {
        ...
        int height = 0;
        for (int i = 0; i < list.size(); i++) {
            height += list.get(i).getHeight(sliceStyle, sliceViewPolicy);
        }
        return height;
    }

    private static SliceItem findHeaderItem(Slice slice) {
        SliceItem sliceItemFind = SliceQuery.find(slice, "slice", (String[]) null, new String[]{"list_item", "shortcut", "actions", "keywords", "ttl", "last_updated", "horizontal", "selection_option"});
        if (sliceItemFind == null || !isValidHeader(sliceItemFind)) {
            return null;
        }
    ...
    private static SliceItem getSeeMoreItem(Slice slice) {
        SliceItem sliceItemFindTopLevelItem = SliceQuery.findTopLevelItem(slice, null, null, new String[]{"see_more"}, null);
        if (sliceItemFindTopLevelItem == null || !"slice".equals(sliceItemFindTopLevelItem.getFormat())) {
            return null;
        }
        List<SliceItem> items = sliceItemFindTopLevelItem.getSlice().getItems();
        return (items.size() == 1 && "action".equals(items.get(0).getFormat())) ? items.get(0) : sliceItemFindTopLevelItem;
    }
