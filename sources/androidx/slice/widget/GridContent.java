...
public class GridContent extends SliceContent {
    ...
    private boolean populate(SliceItem sliceItem) {
        ...
        SliceItem sliceItemFind = SliceQuery.find(sliceItem, (String) null, "see_more", (String) null);
        this.mSeeMoreItem = sliceItemFind;
        if (sliceItemFind != null && "slice".equals(sliceItemFind.getFormat()) && (items = this.mSeeMoreItem.getSlice().getItems()) != null && items.size() > 0) {
        ...
        if ("slice".equals(sliceItem.getFormat())) {
            List<SliceItem> listFilterAndProcessItems = filterAndProcessItems(sliceItem.getSlice().getItems());
            for (int i = 0; i < listFilterAndProcessItems.size(); i++) {
                SliceItem sliceItem2 = listFilterAndProcessItems.get(i);
    ...
    private void processContent(CellContent cellContent) {
        ...
        if (cellContent.isValid()) {
            ...
            if (i == 5) {
                iMax = cellContent.getImageMode();
            } else {
                iMax = Math.max(i, cellContent.getImageMode());
            }
            this.mLargestImageMode = iMax;
        }
    }
