...
public class ActionRow extends FrameLayout {
    ...
    private void createRemoteInputView(int i, Context context) {
        RemoteInputView remoteInputViewInflate = RemoteInputView.inflate(context, this);
        remoteInputViewInflate.setVisibility(4);
        addView(remoteInputViewInflate, new FrameLayout.LayoutParams(-1, -1));
        remoteInputViewInflate.setBackgroundColor(i);
    }
    ...
    boolean handleRemoteInputClick(View view, SliceItem sliceItem, RemoteInput remoteInput) {
        ...
        RemoteInputView remoteInputViewFindRemoteInputView = null;
        for (View parent = view.getParent().getParent(); parent != 0 && (!(parent instanceof View) || (remoteInputViewFindRemoteInputView = findRemoteInputView(parent)) == null); parent = parent.getParent()) {
        }
        ...
        remoteInputViewFindRemoteInputView.setAction(sliceItem);
        remoteInputViewFindRemoteInputView.setRemoteInput(new RemoteInput[]{remoteInput}, remoteInput);
