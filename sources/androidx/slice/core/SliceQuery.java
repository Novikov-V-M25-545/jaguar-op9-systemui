...
public class SliceQuery {
    ...
    private static SliceItem findSliceItem(Deque<SliceItem> deque, Filter<SliceItem> filter) {
        while (!deque.isEmpty()) {
            SliceItem sliceItemPoll = deque.poll();
            if (filter.filter(sliceItemPoll)) {
                return sliceItemPoll;
            }
            if ("slice".equals(sliceItemPoll.getFormat()) || "action".equals(sliceItemPoll.getFormat())) {
                Collections.addAll(deque, sliceItemPoll.getSlice().getItemArray());
            }
        }
        return null;
    }
