...
public class CompatPermissionManager {
    ...
    private synchronized void persist(PermissionState permissionState) {
        getPrefs().edit().putStringSet(permissionState.getKey(), permissionState.toPersistable()).putBoolean(permissionState.getKey() + "_all", permissionState.hasAllPermissions()).apply();
    }
    ...
    public static class PermissionState {
        ...
        boolean addPath(List<String> list) {
            String[] strArr = (String[]) list.toArray(new String[list.size()]);
        ...
        boolean removePath(List<String> list) {
            String[] strArr = (String[]) list.toArray(new String[list.size()]);
            boolean z = false;
            for (int size = this.mPaths.size() - 1; size >= 0; size--) {
